...
server <- function(input, output, session) {
  ...
  observe({
    try({
      ...
      siteDataTable <- siteDataTable[sapply(siteDataTable$VariableCV, function(p) {any(input$VariableCVInput %in% p)}), ]
      ...
      siteDataTable <- siteDataTable %>% filter((minTimeFrameStart >= input$sliderInput[1] | is.na(minTimeFrameStart)), (maxTimeFrameEnd <= input$sliderInput[2] | is.na(maxTimeFrameEnd)))
      siteDataTable$siteLabel  <- siteDataTable$SiteNativeID
      siteDataTable$markColor <- ifelse(siteDataTable$WaDENameWS == "Surface Water", "#0072B2", ifelse(siteDataTable$WaDENameWS == "Groundwater", "#009E73", "#D55E00"))
      siteDataTable$markColor <- ifelse(siteDataTable$CountVar > 0, siteDataTable$markColor, "#000000")
      ...
      SiteMapProxy = leafletProxy(mapId="mapA") %>%
        ...
        addCircleMarkers(
          ...
          popup = paste(
            ...
            "<b>Variable Data Type:</b>", siteDataTable$VariableCV, "<br>",
